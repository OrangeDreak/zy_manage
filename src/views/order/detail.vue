<template>
  ...
  <div class="detail fs14" v-loading="loading">
    <el-card shadow="never">
      <div class="head">
        <el-form class="order-detail-form head-form" :model="orderInfo" inline>
          ...
          <el-form-item label="下单时间:">{{ orderInfo.gmtOrderPay }}</el-form-item>
      ...
      <div class="status">
        <div
          v-if="
            ...
            (orderInfo.businessPayStatus && [2, 3, 4, 5, 6].includes(orderInfo.orderStatus))
          "
          class="con"
        >
          <div class="title">
            <template
              ...
              >待补款</template
            >
            <template v-else>{{ orderInfo.orderStatusDesc }}</template>
            &nbsp;<el-tag
              v-if="
                orderInfo.orderStatus == 14 ||
                (orderInfo.businessPayStatus && [2, 3, 4, 5, 6].includes(orderInfo.orderStatus))
          ...
          <div v-if="countDownShow" class="desc">
            若买家未在
            <span>
              <count-down :end="countDownTime" @countDownEnd="countDownEnd" v-slot="timeObj">
        ...
        <div v-else-if="orderInfo.orderStatus == 2 || orderInfo.orderStatus == 3" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
        </div>
        <!-- <div v-else-if="orderInfo.orderStatus == 3" class="con">
          <div class="title">采购已接单，商品待采购</div>
        </div> -->
        <!-- <div v-else-if="orderInfo.orderStatus == 4" class="con">
          <div class="title">订单已取消</div>
          <div class="desc">取消时间：{{ orderInfo.createTime }}</div>
        </div> -->
        <!-- <div v-else-if="orderInfo.orderStatus == 4" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">
        ...
        <!-- <div v-else-if="orderInfo.orderStatus == 6" class="con">
          <div class="title">用户确认超时，订单已撤单</div>
          <div class="desc">撤单时间：{{ orderInfo.createTime }}</div>
        </div> -->
        <div v-else-if="orderInfo.orderStatus == 5" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">买家确认时间：{{ orderInfo.gmtOperateTime }}</div>
        </div>
        <div v-else-if="orderInfo.orderStatus == 6" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <!-- <div class="desc">发货时间：{{ orderInfo.createTime }}</div> -->
        </div>
        <div v-else-if="orderInfo.orderStatus == 7" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">发货时间：{{ orderInfo.gmtOperateTime }}</div>
        </div>
        <div v-else-if="orderInfo.orderStatus == 8" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">签收时间：{{ orderInfo.createTime }}</div>
        </div>
        <div v-else-if="orderInfo.orderStatus == 9" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">取消原因：{{ orderInfo.cancelReason }}</div>
          <div class="desc">取消时间：{{ orderInfo.gmtOperateTime }}</div>
        </div>
        <div v-else-if="orderInfo.orderStatus == 10" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">撤单时间：{{ orderInfo.gmtOperateTime }}</div>
        </div>
        <!-- <div v-else-if="orderInfo.orderStatus == 14" class="con">
          <div class="title">{{ orderInfo.orderStatusDesc }}</div>
          <div class="desc">
            若买家未在
            <span>
      ...
      <div class="main">
        ...
        <el-row>
          ...
          <el-col :span="6" v-if="[7, 8, 11, 13].includes(orderInfo.orderStatus)">
            <div class="con">
          ...
          <el-col :span="6" v-if="![1, 12, 8, 9].includes(orderInfo.orderStatus)">
            <div class="con">
              <div class="title">付款信息</div>
              <el-form :model="orderInfo" size="">
                <el-form-item label="实付金额:"
                  >{{
                    convertFenToYuan(orderInfo.actualPayAmount) | formatterThousands
                  }}
                  元</el-form-item
                >
      ...
      <div>
                    ...
                    <div>
                      <div class="item-list">
                        ...
                        <div class="num center" :style="{ width: leftColumns[2].width }">
                          ...
                          <div>
                            <el-button
                              v-if="item.skuCount > 1"
                              type="primary"
                              size="mini"
                              class="spilt-order"
                              @click="splitOrder(item)"
                              >拆单</el-button
                            >
                          </div>
                        </div>
        ...
        <div class="count-amout">
          <el-form :model="amoutForm" label-width="100px" class="order-detail-form">
            ...
            >
            <el-form-item v-if="orderInfo.orderStatus > 1" label="补款金额："
              >{{ amoutForm.totalAdditionalPayment }}元</el-form-item
            >
            ...
            >
            <!-- <div>
              <div class="title">实收金额：{{ amoutForm.totalAmount }}</div>
            </div> -->
          </el-form>
        </div>
      </div>
    ...
    <SplitOrderDialog ref="refSplitOrderDialog" @finish="getData"></SplitOrderDialog>
  </div>
...
export default {
  ...
  components: { countDown, ReplyPopover, SplitOrderDialog },
  computed: {
  ...
  methods: {
      ...
      getDetail({ id: this.$route.params.id })
        .then((res) => {
          ...
          this.amoutForm = {
            ...
            totalAdditionalPayment: convertFenToYuan(this.orderInfo.totalAdditionalPayment),
    ...
    splitOrder(item) {
      this.$refs.refSplitOrderDialog.show(item);
    },
    // 留言回复
