...
export default {
  ...
  methods: {
    handlePreview(name) {
      this.$router.push({
        path: `/myiframe/urlPath?name=preview-${name}&src=${website.reportUrl}/preview?_u=blade-${name}`,
      });
    },
    handleDesign(name) {
      this.$router.push({
        path: `/myiframe/urlPath?name=designer-${name}&src=${website.reportUrl}/designer?_u=blade-${name}`,
      });
    },
    rowDel(row) {
